...
export function generateAddresses(
  ...
  const convertedPubKey = ["y", "z"].includes(xpub[0])
    ? convertPubToXPub(xpub)
    : xpub;
...
export function initializeAddressesDerivation(
  folder: Folder,
...
export function generateNextNAddresses(
  folder: Folder,
  folderXPubBranch: FolderXPubBranch,
  count: number
) {
  if (folder.type === FolderType.SIMPLE) return;
