...
export default abstract class AbstractBatchableExplorer implements Explorer {
    ...
    protected async runQueries(addresses: AddressesList): Promise<AddressesBalanceDifference[]> {
        ...
        Object.keys(addresses).forEach(address => requestsDebouncer[address] = requestUniqId);
    ...
    async fetchAndUpdate(addresses: AddressesList): Promise<AddressesBalanceDifference[]> {
        ...
        return (await Promise.all(chunkList.map((addresses) => this.runQueries(addresses)))).flatMap(v => v);
    }
}
