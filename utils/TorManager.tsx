...
export function TorContextProvider({ children }) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      switch (status.toUpperCase().replace(/"/g, "")) {
        case "NOTINIT":
          ourStatus = TorStatusType.DISCONNECTED;
          break;
        case "STARTING":
          ourStatus = TorStatusType.CONNECTING;
          break;
        case "DONE":
          ourStatus = TorStatusType.CONNECTED;
          break;
        default:
          ourStatus = TorStatusType.DISCONNECTED;
          break;
  ...
  return <TorContext.Provider value={context}>{children}</TorContext.Provider>;
}
