...
export default function LockScreen({ children }) {
  ...
  useAppStateEffect(
    (appState, last) => {
      ...
      if (!last) {
        //Lock on first open (should be locked anyway)
        lock();
  ...
  return (
    <LockScreenContext.Provider
      value={{
        ...
        setBiometricUnlocking: (unlocking) => {
          biometricUnlocking.current = unlocking;
        },
      }}
    >
      {children}
...
const FrontLockScreen = ({ onAskUnlock, onAskBiometric }) => {
  ...
  const [biometricType, setBiometricType] = useState(
    LocalAuth.CACHED_AVAILABLE_BIOMETRIC
  );
  ...
  const content = (
    <>
      ...
      >
        ...
        >
          {i18n.t("settings.security.enter_passphrase")}
        </Paragraph>
        <View
          style={{
            display: "flex",
        ...
        >
          {checking ? (
            ...
            <Button onPress={() => onSubmit(input)}>
              {i18n.t("settings.security.unlock")}{" "}
            </Button>
          )}
        </View>
      </View>
  ...
  if (Platform.OS === "android") {
    return (
      ...
      >
        <View style={styles.lockScreenContent}>{content}</View>
      </View>
    );
  }
  return (
    <BlurView intensity={100} style={styles.frontLockScreen}>
      <View style={styles.lockScreenContent}>{content}</View>
    </BlurView>
  );
};
