...
import type { Icons } from "../utils/Types";

declare type DynamicTitleProps = {
  title: string;
  icon?: Icons;
...
export function MainTitle(props: {
  ...
  if (props.icon) {
    mainTitle = (
      <Text numberOfLines={1} style={styles.mainTitle}>
        <Ionicons name={props.icon} size={20} color="white" /> {props.title}
      </Text>
    );
  } else {
    mainTitle = (
  ...
  if (props.onPress) {
    return (
      <TouchableOpacity onPress={props.onPress}>{mainTitle}</TouchableOpacity>
    );
  } else {
    return mainTitle;
  }
